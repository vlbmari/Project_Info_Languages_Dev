document.addEventListener('DOMContentLoaded', () => {
    ...
    async function callComparisonAPI(tech1, tech2) {
        addBotMessage(`🔄 Analisando e comparando ${tech1.nome} e ${tech2.nome}... Aguarde a resposta da IA.`);
        clearInput();

        try {
            // Chama o nosso backend seguro, não a API do Google diretamente
            ...
            addBotMessage(`<b>--- Análise Comparativa da IA ---</b><br>${data.comparison.replace(/\n/g, '<br>')}`);

        } catch (error) {
            console.error('Falha na API de comparação, usando fallback:', error);
            let fallbackMessage = "Desculpe, estamos em manutenção, mas em breve funcionarei melhor! <br><br>Segue abaixo o diferencial das tecnologias informadas:<br><br>";
